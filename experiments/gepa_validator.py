...
def build_examples(owasp_dir, n, seed):
    ...
    findings = findings[:n] if n else findings
    exs = []
    for f in findings:
...
def fp_metric(gold, pred, trace=None, pred_name=None, pred_trace=None):
    ...
    return dspy.Prediction(score=0.2, feedback="MISS (false negative): you dismissed a "
                           f"REAL exploitable bug. Your rationale: {rat}. Do NOT "
                           "over-suppress — find the exploitable path.")
...
def evaluate(module, testset):
    ...
    return {"tp": tp, "fp": fp, "tn": tn, "fn": fn, "bad": bad,
            "precision": prec, "recall": rec, "fp_rate": fpr, "f1": f1}
...
def main():
    ...
    task_lm = dspy.LM("together_ai/meta-llama/Llama-3.3-70B-Instruct-Turbo",
                      api_key=os.environ["TOGETHER_API_KEY"], temperature=0.0, max_tokens=600)
    ...
    print("=== RESULT (held-out FP reduction) ===")
    ...
    print(f"  false positives: {base['fp']} -> {opt['fp']}  (recall guard: recall must not crater)")
